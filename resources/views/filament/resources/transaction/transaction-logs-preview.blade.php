    ...
    @else
        <div class="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700">
            <table class="w-full text-sm">
                <thead class="bg-white/5 text-gray-300 border-b border-gray-200 dark:border-gray-700">
                    <tr class="text-left">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-white/10">
                    @foreach ($logs as $i => $row)
                        <tr class="text-gray-100">
                            ...
                            <td class="px-3 py-2">
                                {{ isset($row['proportion']) ? number_format(((float)$row['proportion']) * 100, 2) . '%' : '—' }}
                            </td>
                            ...
                            <td class="px-3 py-2">{{ isset($row['gross_amount']) ? number_format((float) $row['gross_amount'], 2) : '—' }}</td>
                            <td class="px-3 py-2">{{ isset($row['discount']) ? number_format((float) $row['discount'], 2) : '—' }}</td>
                            <td class="px-3 py-2">{{ isset($row['banking_fee']) ? number_format((float) $row['banking_fee'], 2) : '—' }}</td>
                            <td class="px-3 py-2">{{ isset($row['net_amount']) ? number_format((float) $row['net_amount'], 2) : '—' }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
