                ...
                Section::make('Overview')
                    ...
                    ->headerActions([
                        Html2MediaAction::make('exportPdf')
                            ...
                            ->print(true)      // muestra botón de imprimir
                            ->savePdf()        // habilita descarga en PDF
                            ->filename(fn ($record) => sprintf('OperativeDoc-%s', $record?->id ?? now()->format('Ymd_His')))
                            ->orientation('portrait')
                            ->format('a4', 'mm')
                            ->margin([10, 10, 10, 10]) // top,right,bottom,left
                            // Importante: pasamos el MISMO contenido que renderizas en la sección
                            ->content(function (\Filament\Forms\Get $get, $record, $livewire) {

                                // ---- Calcula el mismo arreglo que retornas en ->viewData(...) ----
                                ...
                                $insureds = collect($get('insureds') ?? [])->map(function ($insured) {
                                    ...
                                    return [
                                        'company'  => $company ? ['name' => $company->name, 'country' => ['name' => optional($company->country)->name]] : ['name' => '-', 'country' => ['name' => '-']],
                                ...
                                foreach ($transactions as $txn) {
                                    ...
                                    $totalConvertedPremium += $rate > 0 ? ($totalPremiumFts * $proportion) / $rate : 1;
                                }
                                ...
                                if ($persistedTxIds->isNotEmpty()) {
                                    $logs = \App\Models\TransactionLog::with('toPartner')
                                        ->whereIn('transaction_id', $persistedTxIds)->get();
                                ...
                                // 👉 devolvemos el mismo view con el mismo payload:
                                return view('filament.resources.business.operative-doc-summary', [
                                    'id'                    => $get('id'),
                                    ...
                                    'premiumType'           => $record?->business?->premium_type ?? $business?->premium_type ?? '-',
                                    'originalCurrency'      => $record?->business?->currency?->acronym ?? $business?->currency?->acronym ?? '-',
